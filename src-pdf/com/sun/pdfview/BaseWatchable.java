...
public abstract class BaseWatchable implements Watchable, Runnable {
	private static final String TAG = BaseWatchable.class.getSimpleName()+"_class";
    ...
    public void run() {
        // Savelog.d(TAG, debug, Thread.currentThread().getName() + " starting");
        ...
        synchronized (parserLock) {
            while (!isFinished() && getStatus() != Watchable.STOPPED) {
                if (isExecutable()) {
                    ...
                    // Savelog.d(TAG, debug, Thread.currentThread().getName() + " waiting: status = " + getStatusString());
        ...
        // Savelog.d(TAG, debug, Thread.currentThread().getName() + " exiting: status = " + getStatusString());
    ...
    protected void setStatus(int status) {
        synchronized (statusLock) {
            ...
            SavelogPDF.d(TAG, debug, " status set to " + getStatusString());
    ...
    protected void setError(Exception error) {
        if (!SuppressSetErrorStackTrace) {
        	SavelogPDF.e(TAG, error.getMessage());
        }
