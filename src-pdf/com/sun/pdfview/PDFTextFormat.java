...
public class PDFTextFormat implements Cloneable {
    ...
    private void doTextNormal(final PDFPage cmds, final String text) {
        ...
        SavelogPDF.d(TAG, debug, "getting font " + font.getBaseFont() + " glyph for text: " + text);
        final List<PDFGlyph> l = (List<PDFGlyph>) font.getGlyphs(text);
        SavelogPDF.d(TAG, debug, "number of glyphs obtained for text " + text + " is " + l.size());

        for (final PDFGlyph glyph : l) {
        	//Savelog.d(TAG, debug, "doTextNormal() adding text by glyph: " + glyph.getChar() + " path=" + glyph.getShape().toString() );
            at.set(cur);
            at.preConcat(scale);
            final PointF advance = glyph.addCommands(cmds, at, tm);

            //Savelog.d(TAG, debug, "doTextNormal() PointF advance: (" + advance.x + "," + advance.y + ")");
