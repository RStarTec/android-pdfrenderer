...
public class FlateDecode {
	private static final String TAG = FlateDecode.class.getSimpleName()+"_class";
    ...
    public static ByteBuffer decode(PDFObject dict, ByteBuffer buf,
        ...
        SavelogPDF.d(TAG, debug, "bufSize=" + bufSize);
        ...
        try {
            while (!inf.finished()) {
                ...
                if (read <= 0) {
                	SavelogPDF.d(TAG, debug, "Read = " + read + "! Params: " + params);
                    if (inf.needsDictionary()) {
                        throw new PDFParseException("Don't know how to ask for a dictionary in FlateDecode");
                    } else {
                    	SavelogPDF.d(TAG, debug, "Inflate data length=" + buf.remaining());
