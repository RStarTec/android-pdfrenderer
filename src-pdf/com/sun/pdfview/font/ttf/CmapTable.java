...
public class CmapTable extends TrueTypeTable {
	private static final String TAG = CmapTable.class.getSimpleName()+"_class";
    ...
    public void addCMap(short platformID, short platformSpecificID,
                        CMap cMap) {
    	SavelogPDF.d(TAG, debug, "addCMap for platformID " + platformID);
    ...
    public void setData(ByteBuffer data) {
        ...
        SavelogPDF.d(TAG, debug, "Data size "+ data.limit()+ " Version: " + getVersion() + " number of subtables: " + numberSubtables);
        ...
        for (int i = 0; i < numberSubtables; i++) {
            ...
            SavelogPDF.d(TAG, debug, "platformID=" + platformID + " offset=" + offset);
            data.mark();
            ...
            try {
                CMap cMap = CMap.getMap(mapData, numGlyphs);
                if (cMap != null) {
                    addCMap(platformID, platformSpecificID, cMap);
                }
            } catch (Exception ex) {
                SavelogPDF.e(TAG, "Error reading map.  PlatformID=" +
                                    platformID + ", PlatformSpecificID=" + 
                                    platformSpecificID);
                SavelogPDF.e(TAG, "Reason: " + SavelogPDF.getStack(ex));
            }
        }
    }
