...
public class FunctionType4 extends PDFFunction {
	...
	protected void parse(PDFObject obj) throws IOException {
    	...
    	SavelogPDF.d(TAG, debug, "byteA (size=" + byteA.length + ") " + scriptContent);
	...
	protected void doFunction(float[] inputs, int inputOffset, float[] outputs, int outputOffset) {
    	...
    	for (Iterator<String> iterator = this.tokens.iterator(); iterator.hasNext(); ) {
			...
			if (debug) { 
				SavelogPDF.d(TAG, debug, "stack before evaluating token " + token);
				printStack(); 
			}
			...
			if (debug) { 
				SavelogPDF.d(TAG, debug, "stack after evaluating token " + token);
				printStack();
			}
		}
	...
	private void prepareInitialStack(float[] inputs, int inputOffset) {
    	...
    	for (int i = inputOffset; i < inputs.length; i++) {
    		this.stack.push(Double.valueOf(inputs[i]));
		}
	}
	...
	private void printTokens() {
		SavelogPDF.i(TAG, "<TokensStart>");
		String tokenValues = "";
    	for (String t : this.tokens) {
    		tokenValues += "<"+ t + ">" + " ";
      	}
  		SavelogPDF.i(TAG, "Tokens: <start> " + tokenValues + "<end>");
	}

	
	private void printStack() {
		String stackValues = "";
    	for (Object t : this.stack) {
    		stackValues += t.toString() + " ";
      	}
  		SavelogPDF.i(TAG, "Stack: <start> " + stackValues + "<end>");
	}
}